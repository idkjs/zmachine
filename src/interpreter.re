...
let handle_or = (a, b, interpreter) => a lor b;

/* Spec: 2OP:9 and a b -> (result)
   Bitwise AND. */

let handle_and = (a, b, interpreter) => a land b;
...
let handle_test_attr = (obj, attr, interpreter) => {
  ...
  if (Object.attribute(interpreter.story, obj, attr)) {
    1;
  } else {
    0;
  };
...
let handle_set_attr = (obj, attr, interpreter) => {
  ...
  {
    ...interpreter,
    story: Object.set_attribute(interpreter.story, obj, attr),
  };
};
...
let handle_clear_attr = (obj, attr, interpreter) => {
  ...
  {
    ...interpreter,
    story: Object.clear_attribute(interpreter.story, obj, attr),
  };
};
...
let handle_print_addr = (addr, interpreter) => {
  let addr = Zstring(addr);
  let text = Zstring.read(interpreter.story, addr);
...
let handle_print_paddr = (packed_address, interpreter) => {
  ...
  let address =
    Story.decode_string_packed_address(interpreter.story, packed_address);
...
let handle_print = (interpreter, instruction) => {
  let printed_interpreter =
    switch (Instruction.text(instruction)) {
...
let handle_pop = interpreter => pop_stack(interpreter);
...
let handle_new_line = interpreter => print(interpreter, "\n");
...
let handle_print_char = (code, interpreter) => {
  let text = string_of_char(char_of_int(code));
  print(interpreter, text);
};
...
let handle_print_num = (value, interpreter) => {
  let value = signed_word(value);
  let text = Printf.sprintf("%d", value);
  print(interpreter, text);
};
...
let step_instruction = interpreter => {
  ...
  | (OP0_185, []) =>
    if (Story.v4_or_lower(Story.version(interpreter.story))) {
      effect(handle_pop);
    } else {
      value(handle_catch);
    }
