...
let global_addr = (story, Global(global)) =>
  if (global < first_global || global > last_global) {
    failwith("global variable index out of range");
  } else {
    let Global_table_base(base) = Story.global_variables_table_base(story);
    let base = Word_address(base);
    let offset = global - first_global;
...
let display = story => {
  let to_string = g =>
    Printf.sprintf("%02x %04x\n", g - first_global, read(story, Global(g)));
